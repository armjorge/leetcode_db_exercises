...
def market_analysis(users: pd.DataFrame, orders: pd.DataFrame, items: pd.DataFrame) -> pd.DataFrame:
        orders = orders[orders['order_date'].dt.year == 2019][['buyer_id', 'order_id']]
        print(orders.head(), '\nÓrdenes')
        grouped = orders.groupby('buyer_id').size().reset_index()
        print(grouped.head())
        df_final = users[['user_id', 'join_date']].merge(grouped, left_on = 'user_id', right_on = 'buyer_id', how = 'left').fillna(0)
        print(df_final.columns)

        df_final.columns = ['buyer_id', 'join_date', 'b_num', 'orders_in_2019']
        return df_final[['buyer_id', 'join_date', 'orders_in_2019']]

df_final = market_analysis(users, orders, items)
